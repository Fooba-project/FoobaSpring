...
public class ResController {
	...
	public String res_FindId(@RequestParam(value="rname" ,required=false)String rname,@RequestParam(value="rphone",required=false)String rphone,Model model) {
		if(rname!=null)model.addAttribute("rname",rname);
		if(rphone!=null)model.addAttribute("rphone",rphone);
		if(rname==null||rname.equals("")||rphone==null||rphone.equals("")) {
			model.addAttribute("message","정보를 모두 입력하세요");
			return  "restaurant/res_FindId";
		}else {
			HashMap<String,Object>paramMap=new HashMap<String,Object>();
	...
	public String res_FindPw(@RequestParam(value="rname",required=false)String rname,@RequestParam(value="rphone",required=false)String rphone,
			@RequestParam(value="rid",required=false)String rid,Model model) {
		if(rid!=null) model.addAttribute("rid",rid);
		if(rname!=null) model.addAttribute("rname",rname);
		if(rphone!=null) model.addAttribute("rphone",rphone);
		if(rid==null||rid.equals("")||rname==null||rname.equals("")||rphone==null||rphone.equals("")) {
			model.addAttribute("message","정보를 모두 입력하세요");
			return  "restaurant/res_FindPw";
		}else {
			HashMap<String,Object>paramMap=new HashMap<String,Object>();
	...
	@ResponseBody	
	public HashMap<String , Object>fileup2(Model model,HttpServletRequest request) throws IOException{
	...
	public String res_review( HttpSession session, Model model, @ModelAttribute("key")int key,
		...
		if(session.getAttribute("x")!=null) {
			model.addAttribute("message","답글을 입력해주세요");
	...
	public String res_orderComplete(HttpSession session, Model model, @RequestParam("OSEQ") int OSEQ){
		HashMap<String , Object> loginRes = (HashMap<String , Object>)session.getAttribute("loginRes");
		if(session.getAttribute("loginRes")==null) return "restaurant/res_login";
		HashMap<String, Object> paramMap = new HashMap<String, Object>();
	...
	public String res_foodmenuUpdateForm(HttpSession session, Model model, @RequestParam("FSEQ") int FSEQ) {
		if(session.getAttribute("loginRes")==null) return "restaurant/res_login";
		HashMap<String , Object> paramMap = new HashMap<String , Object>();
	...
	@RequestParam(value="OLDIMAGE", required=false) String OLDIMAGE) {
		...
		if(procedure.equals("update")) {
			...
			model.addAttribute("messagex","이미지를 넣어주세요.");
			return "restaurant/res_foodMenuAdd";
		}
	...
	@RequestMapping(value="ResImgfileUp", method=RequestMethod.POST) //레스토랑 메뉴 이미지 추가 메서드
	@ResponseBody
	...
	public String res_foodmenuDelete(HttpServletRequest request, HttpSession session, @RequestParam("FSEQ") int FSEQ) {
		if(session.getAttribute("loginRes")==null) return "restaurant/res_login";
		HashMap<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("FSEQ", FSEQ);
